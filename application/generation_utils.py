...
class GenerationMixin:
    ...
    def generate(
        ...
        interpretation: Optional[bool] = False,
        best_indices: Optional[list] = None,
        best_beam_indices: Optional[list] = None,
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            return self.beam_search(
                ...
                best_beam_indices=best_beam_indices,
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            if interpretation:
                model_inputs['encoder_outputs']['last_hidden_state'].retain_grad()
                ...
                self.model.decoder.embed_tokens.register_forward_hook(get_features('embed_tokens'))

            self.zero_grad()
            outputs = self(**model_inputs,return_dict=True,output_attentions=output_attentions,output_hidden_states=output_hidden_states,)
            if interpretation:
                if (cur_len - 1) < len(best_beam_indices) and cur_len < len(best_indices):
                    beam_idx = best_beam_indices[cur_len - 1]
                    token_idx = best_indices[cur_len]


                    target = torch.zeros_like(outputs.logits[beam_idx, -1, :])
                    target[token_idx] = 1
                    (outputs.logits[beam_idx, -1, :] - target).sum().backward(retain_graph=True)
                    input_saliency = (model_inputs['encoder_outputs']['last_hidden_state'][beam_idx] * model_inputs['encoder_outputs']['last_hidden_state'].grad[beam_idx]).mean(-1)
                    output_saliency = (features['embed_tokens'][beam_idx] * features['embed_tokens'].grad[beam_idx]).sum(-1)
                    saliency.append({
                        'input': input_saliency.detach().tolist(),
                        'output': output_saliency.detach().tolist(),
                    })

                # features['embeddings'][beam_idx, -1] * features['embeddings']
                # def ig_forward(inputs, attentions=None):
                #     model_inputs['encoder_outputs']= (inputs)
                #     # model_inputs['encoder_outputs']['attentions'] = attentions
                #     outputs = self(
                #         **model_inputs,
                #         return_dict=True,
                #         output_attentions=output_attentions,
                #         output_hidden_states=output_hidden_states,
                #     )
                #     return outputs.logits[:, -1, :][beam_idx, token_idx]

                # lig = IntegratedGradients(ig_forward)
                # last_hidden_state = model_inputs['encoder_outputs']['last_hidden_state']
                # input_attribution = lig.attribute(last_hidden_state[0].unsqueeze(0), internal_batch_size=1)
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                beam_output = BeamSearchEncoderDecoderOutput(
                ...
                if interpretation:
                    setattr(beam_output, 'saliency', saliency)
                return beam_output
            else:
