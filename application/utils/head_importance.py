...
def get_head_importance_gptneo(model):
    n_layers = model.config.num_layers
    n_heads = model.config.num_heads
    head_size = int(model.config.hidden_size / n_heads)
    importance_scores = np.zeros((n_layers, n_heads))  # M * M

    for i in range(n_layers):
        self_attention = model.transformer.h[i].attn.attention
        importance_scores = compute_taylor_importance_score_gpt_neo(model, self_attention, i,
                                                            n_heads, head_size, importance_scores)
    return importance_scores


def compute_taylor_importance_score_gpt_neo(model, attn, layer_index, n_heads, head_size, importance_scores):
    ...
    for head_idx in leftover_heads:
        ...
        output_W_grad = (attn.out_proj.weight.grad.index_select(1, index) *
                         attn.out_proj.weight.index_select(1, index)) ** 2
        abs_grad_magnitude = query_W_grad.sum() + key_W_grad.sum() + value_W_grad.sum() + output_W_grad.sum()
